...
//!
//! ```text
//! cargo test --release -p nobz-core --test bench_download -- --ignored --nocapture
//! ```
//!
    ...
    tokio::spawn(async move {
        loop {
            ...
            tokio::spawn(async move {
                ...
                if writer.write_all(b"200 nobz-bench ready\r\n").await.is_err() {
                    return;
                }
...
fn tempfile_dir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("nobz-bench-{pid}-{n}"));
