...
use nobz_core::par2;
use nobz_core::postprocess::{PostProcessConfig, PostProcessStatus, post_process};
use nobz_core::unpack;
...
fn temp_dir() -> PathBuf {
    ...
    let d = std::env::temp_dir().join(format!("nobz-rar-test-{}-{n}", std::process::id()));
...
fn normalize_leaves_independent_archives_alone() {
    ...
    assert!(dir.join("lone.000.rar").exists(), "independent rar untouched");
}
...
fn unpack_follows_volume_set_and_reconstructs_file() {
    ...
    assert_eq!(got, FULL_PAYLOAD.as_bytes(), "split file fully reconstructed");
}
...
async fn post_process_fixes_obfuscated_rar_set_with_par2() {
    ...
    std::fs::write(dir.join("release.000.rar"), FIRST_VOL).unwrap();
    std::fs::write(dir.join("release.001.rar"), SUBSEQUENT_VOL).unwrap();
    let par2 = par2::build_par2_set(&[
        (FIRST_VOL, "set.rar"),
        (SUBSEQUENT_VOL, "set.r00"),
    ]);
    ...
    assert!(dir.join("set.rar").exists(), "main volume renamed to set.rar");
...
async fn post_process_part_rar_multivolume_set() {
    ...
    let leftovers: Vec<String> = std::fs::read_dir(&dir)
        ...
        .collect();
    assert!(leftovers.is_empty(), "parts must be cleaned up: {leftovers:?}");
}
...
async fn post_process_fixes_obfuscated_rar_set() {
    ...
    assert_eq!(
        report.status,
        nobz_core::postprocess::PostProcessStatus::UnpackedWithoutVerify
    );
    ...
    let leftovers: Vec<String> = std::fs::read_dir(&dir)
        ...
        .map(|e| e.file_name().to_string_lossy().to_string())
        .collect();
    assert!(leftovers.is_empty(), "rar volumes must be cleaned up: {leftovers:?}");
}
