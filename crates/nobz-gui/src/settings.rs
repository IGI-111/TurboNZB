...
//!
//! Stored as JSON in the OS config directory (`~/.config/nobz/config.json`
//! on Linux, `%APPDATA%\nobz` on Windows, `~/Library/Application Support/nobz`
...
impl AppConfig {
    ...
    pub fn defaults(dirs: &directories::ProjectDirs) -> Self {
        ...
        let download_dir = downloads_base.join("nobz");
        let completed_dir = download_dir.join("completed");
        Self {
            servers: Vec::new(),
            indexers: Vec::new(),
            db_path: data_dir.join("nobz-queue.db"),
            download_dir,
            completed_dir,
            categories: vec![
                CategoryMapping {
