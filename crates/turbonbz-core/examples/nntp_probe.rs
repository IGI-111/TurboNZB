...
//!
//! Reads the first server from the real config (`~/.config/turbonbz/config.json`
...
//! Usage:
//! ```text
//! cargo run -p turbonbz-core --example nntp_probe -- --conns 8 --seconds 10
//! cargo run -p turbonbz-core --example nntp_probe -- --config /path/to/config.json
//! ```
...
use turbonbz_core::nntp::{NntpClient, ServerConfig};
