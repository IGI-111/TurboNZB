...
//!
//! Usage:
//!   turbonbz-cli download --nzb <path> --out <dir> --host <host> --port <port> \
//!     [--tls] [--user <user> --pass <pass>] [--connections N]
//!   turbonbz-cli list
//!   turbonbz-cli resume --job <id> --host <host> --port <port> \
//!     [--tls] [--user <user> --pass <pass>] [--connections N]
//!
//! The queue DB is stored at `--db` (default: `turbonbz-queue.db` in the current
//! directory). Jobs survive restarts — run `turbonbz-cli resume` to continue a
...
use turbonbz_core::postprocess::{PostProcessConfig, post_process};
use turbonbz_core::queue::QueueManager;

#[derive(Parser)]
#[command(name = "turbonbz-cli", about = "Download harness for turbonbz")]
struct Cli {
    /// Path to the queue database.
    #[arg(long, default_value = "turbonbz-queue.db")]
    db: PathBuf,
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::Pause { job } => {
            queue
                .set_job_state(job, turbonbz_core::queue::JobState::Queued)
                .await?;
                ...
                Ok(report) => {
                    if let Some(vr) = &report.verify {
                        ...
                        for (filename, status) in &vr.files {
                            if status != &turbonbz_core::par2::VerifyStatus::Ok {
