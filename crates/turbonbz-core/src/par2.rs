...
pub fn build_par2_set(files: &[(&[u8], &str)]) -> Vec<u8> {
    ...
    let creator_body = b"turbonbz test\0\0\0";
...
pub(crate) mod tests {
    ...
    fn test_parse_and_verify_healthy_file() {
        ...
        let tmp = std::env::temp_dir().join("turbonbz-par2-test-healthy");
    ...
    fn test_verify_missing_file() {
        ...
        let tmp = std::env::temp_dir().join("turbonbz-par2-test-missing");
        std::fs::create_dir_all(&tmp).unwrap();
    ...
    fn test_verify_matches_renamed_file_by_content() {
        ...
        let tmp = std::env::temp_dir().join("turbonbz-par2-test-renamed");
        std::fs::create_dir_all(&tmp).unwrap();
    ...
    fn test_verify_damaged_file() {
        ...
        let tmp = std::env::temp_dir().join("turbonbz-par2-test-damaged");
        std::fs::create_dir_all(&tmp).unwrap();
