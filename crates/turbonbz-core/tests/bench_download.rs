...
//!
//! ```text
//! cargo test --release -p turbonbz-core --test bench_download -- --ignored --nocapture
//! ```
//!
...
use turbonbz_core::nzb::{self, Nzb};
    ...
    tokio::spawn(async move {
        loop {
            ...
            tokio::spawn(async move {
                ...
                if writer
                    .write_all(b"200 turbonbz-bench ready\r\n")
                    .await
                    .is_err()
                {
                    return;
...
fn tempfile_dir() -> PathBuf {
    ...
    let dir = std::env::temp_dir().join(format!("turbonbz-bench-{pid}-{n}"));
