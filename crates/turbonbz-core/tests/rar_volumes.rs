...
use turbonbz_core::par2;
use turbonbz_core::postprocess::{PostProcessConfig, PostProcessStatus, post_process};
use turbonbz_core::unpack;
...
fn temp_dir() -> PathBuf {
    ...
    let d = std::env::temp_dir().join(format!("turbonbz-rar-test-{}-{n}", std::process::id()));
...
async fn post_process_fixes_obfuscated_rar_set() {
    ...
    assert_eq!(
        report.status,
        turbonbz_core::postprocess::PostProcessStatus::UnpackedWithoutVerify
    );
