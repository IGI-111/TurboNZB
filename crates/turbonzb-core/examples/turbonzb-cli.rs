...
//!
//! Usage:
//!   turbonzb-cli download --nzb <path> --out <dir> --host <host> --port <port> \
//!     [--tls] [--user <user> --pass <pass>] [--connections N]
//!   turbonzb-cli list
//!   turbonzb-cli resume --job <id> --host <host> --port <port> \
//!     [--tls] [--user <user> --pass <pass>] [--connections N]
//!
//! The queue DB is stored at `--db` (default: `turbonzb-queue.db` in the current
//! directory). Jobs survive restarts — run `turbonzb-cli resume` to continue a
...
use turbonzb_core::postprocess::{PostProcessConfig, post_process};
use turbonzb_core::queue::QueueManager;

#[derive(Parser)]
#[command(name = "turbonzb-cli", about = "Download harness for turbonzb")]
struct Cli {
    /// Path to the queue database.
    #[arg(long, default_value = "turbonzb-queue.db")]
    db: PathBuf,
...
async fn main() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Command::Pause { job } => {
            queue
                .set_job_state(job, turbonzb_core::queue::JobState::Queued)
                .await?;
                ...
                Ok(report) => {
                    if let Some(vr) = &report.verify {
                        ...
                        for (filename, status) in &vr.files {
                            if status != &turbonzb_core::par2::VerifyStatus::Ok {
