...
use tracing::warn;
use turbonbz_core::engine::{Engine, ProgressEvent};
use turbonbz_core::nntp::ServerConfig;
use turbonbz_core::nzb;
use turbonbz_core::postprocess::{
    PostProcessConfig, PostProcessReport, PostProcessStatus, post_process,
};
use turbonbz_core::queue::{JobState, QueueJob, QueueManager};
use turbonbz_index::types::{IndexerConfig, SearchQuery};
use turbonbz_index::{AggregatedResult, NewznabClient, SearchAggregator};
...
pub struct JobSegmentDetail {
    pub number: u32,
    pub bytes: u64,
    pub state: turbonbz_core::queue::SegmentState,
}
...
impl Backend {
    ...
    async fn handle_open_nzb_file(&self, path: PathBuf) {
        let title = path
            ...
            .unwrap_or_else(|| "turbonbz-download".into());
    ...
    async fn handle_download_from_url(
        ...
        tokio::spawn(async move {
            ...
            let client = match reqwest::Client::builder()
                ...
                .user_agent(concat!("turbonbz/", env!("CARGO_PKG_VERSION")))
                .build()
            {
                Ok(c) => c,
                Err(e) => {
    ...
    async fn handle_test_server(&self, config: ServerConfig) {
        ...
        match turbonbz_core::nntp::NntpClient::connect(&config).await {
            Ok(_) => {
...
async fn build_job_details(
    queue: &QueueManager,
    _files: &[turbonbz_core::queue::QueueFile],
    job_id: i64,
) -> Vec<JobFileDetail> {
    ...
    for file in &files {
        ...
        let done = segments
            .iter()
            .filter(|s| s.state != turbonbz_core::queue::SegmentState::Pending)
            .count() as u32;
        let missing = segments
            .iter()
            .filter(|s| s.state == turbonbz_core::queue::SegmentState::Missing || s.missing)
            .count() as u32;
        let total_bytes = segments.iter().map(|s| s.bytes).sum();
        let down = segments
            .iter()
            .filter(|s| s.state == turbonbz_core::queue::SegmentState::Done)
            .map(|s| s.bytes)
            .sum();
