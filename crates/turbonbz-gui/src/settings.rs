...
//!
//! Stored as JSON in the OS config directory (`~/.config/turbonbz/config.json`
//! on Linux, `%APPDATA%\turbonbz` on Windows, `~/Library/Application Support/turbonbz`
...
use turbonbz_index::types::IndexerConfig;
...
impl AppConfig {
    ...
    pub fn defaults(dirs: &directories::ProjectDirs) -> Self {
        ...
        let download_dir = downloads_base.join("turbonbz");
        let completed_dir = download_dir.join("completed");
        Self {
            servers: Vec::new(),
            indexers: Vec::new(),
            db_path: data_dir.join("turbonbz-queue.db"),
            download_dir,
            completed_dir,
            categories: vec![
                CategoryMapping {
